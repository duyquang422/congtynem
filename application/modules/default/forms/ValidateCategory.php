<?php
class Default_Form_ValidateCategory{
	...
	public function uploadFile($filename = null, $options = null){
		...
		if(!empty($fileName)){
			...
			$thumb	->resize(587,354)	->save($upload_dir . '/images587x354/' . $fileName);			
			$thumb = Zendvn_File_Images::create($upload_dir . '/orignal/' . $fileName);
			$thumb	->resize(194,177)	->save($upload_dir . '/images194x177/' . $fileName);
						
			if($this->_arrData['action']=='edit')
			{	
				$upload->removeFile($upload_dir . '/orignal/'. $this->_arrData['current_stadium_' . $filename]);
				$upload->removeFile($upload_dir . '/images194x177/'. $this->_arrData['current_stadium_' . $filename]);
				$upload->removeFile($upload_dir . '/images587x254/'. $this->_arrData['current_stadium_' . $filename]);						
			}
		}
		return $fileName;		
	}
