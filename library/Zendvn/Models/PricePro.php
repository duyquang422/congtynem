...
class Zendvn_Models_PricePro extends Zend_Db_Table {
    ...
    public function listItem($arrParam = null, $options = null) {
        ...
        if ($options['task'] == 'view-cart') {
            if (count($arrParam['cart']) > 0) {
                ...
                $select = $db->select()
                        ->from('products AS p', array('id', 'name','alias', 'code', 'picture', 'size', 'price', 'selloff'))
                        ->where('p.status = 1')
                        ->where('p.id IN (' . $ids . ')');
                $result = $db->fetchAll($select);
            }
    ...
    public function priceSelectbox($id) {
        $db = Zend_Registry::get('connectDb');
        //$db = Zend_Db::factory($adapter, $config);
            $select = $db->select()
                    ->from('products AS p', array('id', 'name'))
                    ->joinLeft('price_pro as pp', 'p.code = pp.code_pro', array('pp.size as sizeProduct', 'pp.price'))
                    ->where('p.status = 1')
                    ->where('p.id = ?',$id)
                    ->order('pp.id DESC');
            $result = $db->fetchAll($select);
