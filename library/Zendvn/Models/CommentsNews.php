<?php
class Zendvn_Models_CommentsNews extends Zend_Db_Table{
	protected $_name = 'comment_news';
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'admin-list'){
			$select = $db	->select()
							...
							->joinLeft("news_item AS u","u.id = n.news_id",array("u.name AS product_name"))
							->order('id DESC');	
		...
		if($options['task'] == 'list'){
			$select = $db->select()
						 ->from($this->_name . ' AS n',array('id','user_name','created','status','title','content','special','news_id'))
						 ->joinLeft('news_item AS u','u.id = n.news_id',array('u.name AS news_name'))
						 ->where('n.news_id = ?', $arrParam['id'])
						 ->where('n.status = 1')
						 ->order('n.id DESC')
						 ->limit(15);
	...
	public function saveItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'add-comment'){
			...
			if(!empty($user['user_name'])){
				...
				$row->user_name 	= $arrParam['full_name'];
			}
			if(!empty($user['email'])){
